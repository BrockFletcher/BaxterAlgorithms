...
%                         
            for stella=1:length(myStruct)
                ...
                wellnumber=fname+1;
%                  wellname=matlab.lang.makeValidName(strcat('w',wellnumber));
%                  Time{i}=matlab.lang.makeValidName(i);
%                 Pass{stella}=matlab.lang.makeValidName(stella);
                AllData.(wellname).(timename).area= area;
                for milo=1:length(numPlanes)
                    ...
                    AllData.(wellname).(timename).(SumInt_s)= SumInt;
                     AllData.(wellname).(timename).(InvInt_s)= InvInt;
