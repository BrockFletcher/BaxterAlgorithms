...
%                         
            for stella=1:length(myStruct)
                ...
                Well{j+1}=matlab.lang.makeValidName(j);
                Time{i+1}=matlab.lang.makeValidName(i);
                Pass{stella}=matlab.lang.makeValidName(stella);
                AllData.(Well{j+1}).(Time{i+1}).(Pass{stella}).area = area;
                for milo=1:length(numPlanes)
                    DataImage=Img2(:,:,milo);
                    SumInt=sum(DataImage(myStruct{1,stella}.bw4));
                    InvInt=sum(DataImage(~myStruct{1,stella}.bw4));
                    SumIntensity{milo}=matlab.lang.makeValidName(milo);
                    AllData.(Well{j+1}).(Time{i+1}).(Pass{stella}).SumIntensity{milo} = SumInt;
                   
                    %AllData{j+1,i+1}.InvInt{milo,stella}=InvInt;
                end
