...
%                     end
    ...
    %% Data Segment
            ...
            for stella=1:length(my.Struct)
                
                area= nnz(my.Struct{1,stella}.bw4);
                for milo=1:length(numPlanes)
                    DataImage=Img2(:,:,milo);
                    Sum=DataImage(my.Struct{1,stella}.bw4);
                    Data.well{j}.time{i}.sum=Sum;
                end
                Data.well{j}.time{i}.area=area;
            end
    %% ExportSegment        
